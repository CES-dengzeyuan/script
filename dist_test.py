    ...
    for db in dblist:
            ...
            for batch_size in range(20, 40, 20):
                ...
                if db == 'Aria':
                    command = "./bench_tpcc --log_dir=" + logdir + " --id=" + str(
                        mid) + " --servers='" + ips + "' --protocol=Aria --partition_num=108 --threads=5 --batch_size=" \
                              + str(batch_size) + " --query=" + wkld
                else:
                    command = "./bench_tpcc --log_dir=" + logdir + " --id=" + str(
                        mid) + " --servers='" + ips + "' --protocol=Calvin --partition_num=108 --threads=5 --batch_size=" + str(
        ...
        for thread in range(2, 3):
            for batch_size in range(20, 40, 20):
                ...
                if db == 'Aria':
                    command = "./bench_tpcc --log_dir=" + logdir + " --id=" + str(
                        mid) + " --servers='" + ips + "' --protocol=Aria --partition_num=108 --threads=" + str(
                        thread) + " --batch_size=" + str(batch_size) + " --query=neworder"
                else:
                    command = "./bench_tpcc --log_dir=" + logdir + " --id=" + str(
                        mid) + " --servers='" + ips + "' --protocol=Calvin --partition_num=108 --threads=" + str(
