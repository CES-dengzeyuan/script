    ...
    for db in dblist:
        ...
        for cr in cross_ratios:
            for batch_size in range(20, 520, 20):
                ...
                command = "./bench_tpcc --log_dir=" + logdir + " --id=" + str(mid) + " --servers='" + ips + "' --protocol=Calvin --partition_num=324 --threads=5 --batch_size=" + str(batch_size) + " --query=neworder --replica_group=3 --neworder_dist=" + str(cr)
                print(command)
                os.system(command)
                port += 1
        ...
        for cr in cross_ratios:
            for batch_size in range(20, 520, 20):
                ...
                os.system('mkdir ' + logdir)
                ips = ip0 + ":" + str(port) + ";" + ip1 + ":" + str(port) + ";" + ip2 + ":" + str(port)
                command = "./bench_tpcc --log_dir=" + logdir + " --id=" + str(mid) + " --servers='" + ips + "' --protocol=Calvin --partition_num=324 --threads=5 --batch_size=" + str(batch_size) + " --query=neworder --replica_group=3 --payment_dist=" + str(cr)
                print(command)
                os.system(command)
                port += 1
        # impact of threads
